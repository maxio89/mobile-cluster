...
object Metrics {
  ...
  val addressReads: Reads[Address] = Reads { js =>
    ...
    val address = (host, port) match {
      case (Some(_host), Some(_port)) => Address(proto, system, _host, _port)
  ...
  val addressWrites: Writes[Address] = Writes { a =>
    Json.obj("protocol" -> a.protocol, "system" -> a.system, "host" -> a.host, "port" -> a.port, "full" -> a.toString)
  }
  ...
  val nodeMetricReads: Reads[NodeMetric] = Reads { case js if (js \ "name") == "cpu" => heapMemoryFormat reads js
  case js if (js \ "name") == "heap" => cpuFormat reads js
  }

  val nodeMetricWrites: Writes[NodeMetric] = Writes { case m: HeapMemory => heapMemoryFormat writes m
  ...
  case class Cpu(address: Address, timestamp: Long, systemLoadAverage: Option[Double], cpuCombined: Option[Double], processors: Int,
                 name: String = "cpu") extends NodeMetric

  case class HeapMemory(address: Address, timestamp: Long, used: Long, committed: Long, max: Option[Long], name: String = "heap") extends NodeMetric
